...
void* thr_receiver(void* a)
{
    ...
    struct smlt_qp *qp = &queue_pairs[1][arg->r+1];
...
int main(int argc, char **argv)
{
    ...
    size_t chan_per_core = (NUM_CHANNELS / (num_cores - 1));
    size_t chan_max = chan_per_core * (num_cores - 1);
    size_t current_chan_cnt = 0;
    for (size_t s=0; s<chan_max; s++) {
        ...
        if (++current_chan_cnt == chan_per_core) {
            current_chan_cnt = 0;
            core++;
        }
    }
