 ...
 */
int smlt_tree_generate(uint32_t ncores, uint32_t *cores, const char* tree_name,
                       uint16_t** model, uint32_t *num_leafs, uint32_t** leafs,
                       uint32_t* t_root, uint32_t* len_model);
 ...
 * @return 0 if successful otherwise > 0
 */
int smlt_tree_parse(const char* json_string, uint32_t ncores, uint16_t** model,
                    uint32_t *num_leafs, uint32_t** leafs, uint32_t* t_root,
                    uint32_t* len_model);
