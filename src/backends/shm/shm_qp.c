...
static struct shm_context* shm_init_context(void* shm,
                                            uint8_t node)
{
    ...
    q->num_slots = (DEFAULT_SHM_Q_SIZE)-1; 
...
static void get_next_sync(struct shm_context* q, 
                   uint64_t* next)
{
...
static bool shm_receive_non_blocking(struct shm_context* context,
              uintptr_t *p1,
              uintptr_t *p2,
              uintptr_t *p3,
              uintptr_t *p4,
...
static bool shm_receive_non_blocking0(struct shm_context* context)
{
    //assert (context!=NULL);
    uintptr_t* start;
    start = (uintptr_t*) context->data + ((context->l_pos)*
